...
import org.jetbrains.bio.ext.PathExtensionsKt;
...
public class TestOrganismDataGenerator {
  ...
  final static Map<String, Integer> CHROMOSOMES = ImmutableMap.of(
      "chr1", (int) 1e7,
      "chr2", (int) 1e6,
      "chr3", (int) 1e6,
      "chrX", (int) 1e6,
  ...
  public static final int MAX_GAPS_IN_CHR = 10;
  public static final int MAX_GAP_LENGTH_BP = 100;

  public static void main(final String[] args) throws IOException {
    final Genome genome = Genome.Companion.invoke("to1");
  ...
  public static void createAnnotations(final Genome genome) throws IOException {
    final Path genesPath = genome.getDataPath().resolve("genes.json.gz");

    final Gson gson = Genes.GSON;
    try (final BufferedWriter w = PathExtensionsKt.bufferedWriter(genesPath);
      ...
      for (int i = 0; i < names.size(); i++) {
        ...
        final Chromosome chromosome = Chromosome.Companion.invoke(genome.getBuild(), name);
        final int length = CHROMOSOMES.get(name);
        ...
        while (length - currentEnd > 6e3) {
          ...
          final Gene gene
              = createGeneAnnotation(geneName, chromosome, strand, currentStart, currentLength);
  ...
  public static Gene createGeneAnnotation(final String geneName,
  ...
  public static void createGaps(final Genome genome) {
    final Path gapsPath = genome.getDataPath().resolve(Gaps.GAPS_FILE_NAME);
    try (BufferedWriter w = PathExtensionsKt.bufferedWriter(gapsPath)) {
  ...
  public static void createCytoBands(final Genome genome) {
    final Path cytoBandsPath = genome.getDataPath().resolve(CytoBands.CYTOBANDS_FILE_NAME);
    try (BufferedWriter w = PathExtensionsKt.bufferedWriter(cytoBandsPath)) {
