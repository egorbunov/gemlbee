...
abstract class Column<T> protected constructor(
    ...
    /**
     * Similar to [.append], but only adds the values not yet
     * present in `data`.
     */
     ...
     */
    abstract fun sorted(order: SortOrder): IntArray
    ...
    internal abstract fun load(row: Int, value: String)

    internal open fun typeName() = boxedType.simpleName

    /**
     * Checks if a column can unbox and store a given value.
     */
    fun accepts(value: Any) = boxedType.isInstance(value)
...
interface ObjIntPredicate<T> {
    ...
    infix fun and(other: ObjIntPredicate<in T>): ObjIntPredicate<T> {
        checkNotNull(other)
        return Companion { t, value -> test(t, value) && other.test(t, value) }
    }
