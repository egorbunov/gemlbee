...
import org.jetbrains.bio.genome.query.GenesQuery
...
class GenesTrackView : TrackView(GeneClass.ALL.description) {
    companion object {
        ...
        val ALL_GENES = CacheBuilder.newBuilder().weakKeys().build<GenomeQuery, List<Gene>>()
        fun getGenes(model: BrowserModel): List<Gene> =
                ALL_GENES[model.genomeQuery, { GenesQuery(GeneClass.ALL).process(model.genomeQuery)}]
        ...
        fun determineLabelBgDim(labelX: Int, labelWidth: Int, strand: Strand, geneStartX: Int, geneEndX: Int)
                = when {
            strand.isPlus() -> labelX + labelWidth > geneStartX
            else -> labelX < geneEndX
        }
    }
    ...
    override fun paintTrack(g: Graphics, model: SingleLocationBrowserModel, conf: Storage) {
        ...
        if (genesCount > 1 && selectedGene != null) {
    ...
    private fun transcriptColor(gene: Gene, ignored: Boolean)
            = when {
